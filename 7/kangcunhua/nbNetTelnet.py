...
# -*- coding: utf-8 -*-
# @Author: Kang.Cunhua
# @Date:   2015-11-28 23:48:13
# @Last Modified by:   Kang.Cunhua
# @Last Modified time: 2015-11-29 00:25:34

"""
作业：
    1. nbNetFramework 抄完 加注释
    2. 改成5个字节协议头
    3. 支持Telnet协议 \r\n结尾 没有头那种
    4. 支持HTTP
"""
...
class nbNetBase:
    ...
    def read(self, fd):
        ...
        try:
            ...
            # 判断是否独到了命令结尾
            if sock_state.buff_read.find('\r\n'):
                sock_state.need_read = 0

            # # 如果已经读取的数据是10个byte，那么说明数据的10字节头已经读取完毕，
            # # 我们可以解析判断后续的数据的长度了
            # if sock_state.have_read == 2:
...
class nbNet(nbNetBase):
    ...
    def process(self, fd):
        ...
        # 组装给Client回应的10字节协议头
        sock_state.buff_write = "%s" % (response)
