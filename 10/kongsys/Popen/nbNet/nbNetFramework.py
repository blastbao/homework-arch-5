...
class nbNetBase:
    ...
    def read(self, fd):
        ...
        try:
            ...
            if sock_state.have_read == 10:
                ...
                if sock_state.buff_read[-4:] == '\r\n\r\n':
                    sock_state.buff_read = sock_state.buff_read[:-4]
                    return 'processRPC'
                else:
    ...
    def writeend(self, fd):
        sock_state = self.conn_state[fd]
        subprocess_obj = sock_state.subp_obj
        #Popen not end, call poll() return None else return integer
        if subprocess_obj.poll() == None:
            return "writemore"
        else:
            conn = sock_state.sock_obj
            try:
...
class nbNet(nbNetBase):
    def __init__(self, addr, port, logic):
        ...
        self.sm = {
            ...
            "processRPC": self.processRPC,
            "closing": self.close,
            "writeend": self.writeend2read,
        }
    ...
    def read2process(self, fd):
        ...
        try:
            ...
            #dbgPrint(msg)
            read_ret = "closing"
        if read_ret == "processRPC":
            self.processRPC(fd)
        elif read_ret == "process":
            # recv complete, change state to process it
    ...
    #@profile
    def writeend2read(self, fd):
        write_ret = self.writeend(fd)
