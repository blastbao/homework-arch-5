...
class nbNet(nbNetBase):
    def __init__(self, addr, port, logic):
        ...
        self.sm = {
            ...
            'pending':self.check_pipe_status,
            "closing": self.close,
        }
    ...
    def process(self, fd):
        ...
        if isinstance(response,subprocess.Popen):
            sock_state.pipe_obj = response
            sock_state.state = 'pending'
            self.check_pipe_status(fd)
    ...
    def check_pipe_status(self,fd):
            if self.conn_state[fd].pipe_obj.poll() == 0:
                 try:
                    conn = self.conn_state[fd].sock_obj
                    conn.send(end_tag)
                    self.setFd(conn)
                    self.conn_state[fd].state = "read"
                    self.epoll_sock.modify(fd, select.EPOLLIN)
